...
- Мок Speaches API (httpx responses для VAD, embeddings, transcriptions)
- WAV-файл 16kHz mono (короткий, в памяти)
- Embedding-векторы (нормализованные float-списки)
- Минимальный config.yaml (temp file)

#### Unit-тесты (чистые функции)

**`tests/test_models.py`** — `pipeline/models.py`:
- `merge_close_segments()` — empty, single, merge same speaker, skip different speakers, max_chunk_ms limit, gap > max_gap_ms
- `merge_by_proximity()` — то же но без учёта спикера, speaker preserved from first

**`tests/test_clustering.py`** — `pipeline/clustering.py`:
- `cluster_embeddings()` — 2 голоса → 2 кластера, 1 голос → 1 кластер, пороги влияют на кол-во кластеров

**`tests/test_identification.py`** — `pipeline/embeddings.py` (локальная логика):
- `SpeakerIdentifier.identify()` — direct match (sim ≥ threshold), confident gap accept, below min_threshold reject, no voiceprints → None
- `SpeakerIdentifier._find_nearest_labeled()` — left/right neighbor, both sides, no neighbors
- `SpeakerIdentifier.identify_segments()` — all known, all unknown → clusters, mixed, short segments inherit from neighbor
- `enroll_samples()` — копирует файлы в enrolled_dir, включает ранее зарегистрированные семплы, возвращает correct count

**`tests/test_config.py`** — `servers.py`:
- `load_config()` — file not found, empty file, валидный YAML, дефолты при отсутствии секций
- `AppConfig.validate()` — пустой servers list, несуществующий server name
- **Дефолты совпадают с config.yaml** — парсим оба, сравниваем значения (git-история: 3 бага из-за расхождений)

#### Функциональные тесты (БД + файловая система)

**`tests/test_database.py`** — `database.py` с in-memory SQLite:
- Миграции: идемпотентность (повторный `get_db()`)
- `_validate_team_name()` — валидные/невалидные имена
- Team: create + delete, защита "default" от удаления, cascade к voiceprints
- Voiceprint: save (insert + upsert), load по team, delete
- **Voiceprint model metadata** — save хранит именно `embeddings.model`, не `transcription.model` (git: commit 4242478)
- Auth sessions: create, get с проверкой expiry, delete_expired
- `ensure_default_team()` — идемпотентность

**`tests/test_team.py`** — `team.py`:
- `resolve_team()` — авто-создание "default", резолв существующей команды, ошибка на несуществующую

**`tests/test_session.py`** — `web/services/session.py` (БД + temp dirs):
- Lifecycle: create → add track → configure → delete (cleanup файлов)
- Track: renumbering при удалении средней дорожки
- Sample: add → move между спикерами → delete (cleanup файлов)
- Rollback: ошибка при add_track → файл не остаётся
- **Enrollment копирует семплы на диск** (git: commit 4242478 — web не копировал)

#### Интеграционные тесты (мок HTTP)

**`tests/test_pipeline_integration.py`** — полный flow с моками Speaches:
- VAD: мок → конкретные сегменты с ожидаемыми start_ms/end_ms; мок ошибки → raise
- Embeddings: `extract_segments()` — параллельное извлечение, фильтрация коротких сегментов (< min_duration_ms)
- Transcriber: `transcribe_segments()` — merge → slice → transcribe → проверяем offset timestamps и speaker assignment
- Diarization: VAD → Embeddings → Clustering → Identification — end-to-end, проверяем финальные speaker labels
- `_find_speaker()` — overlap calculation, no overlap → "Unknown"
...
- `SpeachesAPIError` — ошибки удалённого API
- `PipelineError` — ошибки обработки
- `ConfigurationError` — невалидная конфигурация
- HTTP retry для transient failures (httpx retries)

### Версионирование миграций БД

- Таблица `schema_version` + нумерованные миграции
- Замена текущего `CREATE TABLE IF NOT EXISTS` (не поддерживает ALTER)

### Structured logging

- Тайминги: VAD took X ms, embedding extraction took Y ms, transcription took Z ms
